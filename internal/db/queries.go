...
_, err := h.DB.Exec(`
INSERT INTO songs (id,title,artist,album,album_artist,genre,fmt,dur,path,provider_id,cover_path,cover_art,lyrics)
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
...
rows, err := h.DB.Query(`SELECT id,title,artist,album,album_artist,genre,fmt,dur,path,provider_id,cover_path,cover_art,lyrics,plays FROM songs ORDER BY updated_at DESC`)
if err != nil {
return nil, err
}
defer rows.Close()
...
for rows.Next() {
var s Song
if err := rows.Scan(&s.ID, &s.Title, &s.Artist, &s.Album, &s.AlbumArtist,
&s.Genre, &s.Fmt, &s.Dur, &s.Path, &s.ProviderID, &s.CoverPath,
&s.CoverArt, &s.Lyrics, &s.Plays); err != nil {
return nil, err
}
out = append(out, s)
}
...
row := h.DB.QueryRow(`SELECT id,title,artist,album,album_artist,genre,fmt,dur,path,provider_id,cover_path,cover_art,lyrics,plays FROM songs WHERE id=?`, id)
var s Song
if err := row.Scan(&s.ID, &s.Title, &s.Artist, &s.Album, &s.AlbumArtist,
&s.Genre, &s.Fmt, &s.Dur, &s.Path, &s.ProviderID, &s.CoverPath,
&s.CoverArt, &s.Lyrics, &s.Plays); err != nil {
return nil, err
}
...
rows, err := h.DB.Query(`
SELECT s.id,s.title,s.artist,s.album,s.album_artist,s.genre,s.fmt,s.dur,s.path,s.provider_id,s.cover_path,s.cover_art,s.lyrics,s.plays
...
for rows.Next() {
var s Song
rows.Scan(&s.ID, &s.Title, &s.Artist, &s.Album, &s.AlbumArtist,
&s.Genre, &s.Fmt, &s.Dur, &s.Path, &s.ProviderID, &s.CoverPath,
&s.CoverArt, &s.Lyrics, &s.Plays)
out = append(out, s)
}
return out, nil
}
...
func (h *Holder) UpdateSong(id string, fields map[string]string) error {
	allowed := map[string]bool{
		...
		"genre": true, "lyrics": true, "cover_art": true,
	}
	var sets []string
	args := []any{}
